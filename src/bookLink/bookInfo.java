...
public class bookInfo extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					bookInfo frame = new bookInfo(null, "USERNAME");
	...
	public bookInfo(Book selectedBook, String loginUser) {
		        ...
		        if (response == JOptionPane.YES_OPTION) { 
		            ...
		            if (conn.con != null) {
		                ...
		                try (PreparedStatement pst = conn.con.prepareStatement(sql)) {
		                	pst.setInt(1, selectedBook.bookId);
	                        int rowsAffected = pst.executeUpdate();
	                        if (rowsAffected > 0) {
	                            JOptionPane.showMessageDialog(null, "Book deleted successfully.");
	                            new homeAdmin().refreshBookList();
