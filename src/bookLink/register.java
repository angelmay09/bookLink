...
public class register extends JFrame {
	...
	public void addtoDatabase() {
		...
		conn.connect();
		if (conn.con != null) {
            String sql = "INSERT INTO users (username, password, contact, address) VALUES (?, ?, ?, ?)";
            try (PreparedStatement pst = conn.con.prepareStatement(sql)){
                	pst.setString(1, registerUser.getText());
                    pst.setString(2, new String(registerPass.getPassword()));
                    pst.setInt(3, Integer.parseInt(registerContact.getText().trim()));
                    pst.setString(4, registerAddress.getText());
                    pst.executeUpdate();
                    JOptionPane.showMessageDialog(register.this, "Registered successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                    new login().setVisible(true);
                    dispose();
            } catch (NumberFormatException e) {
                JOptionPane.showMessageDialog(this, "Invalid contact number format. It must be a valid integer.", "Input Error", JOptionPane.ERROR_MESSAGE);
            } catch (SQLException e) {
            	JOptionPane.showMessageDialog(register.this, "Error connecting to the database.", "Database Error", JOptionPane.ERROR_MESSAGE);
                e.printStackTrace();
            }
        }
